...
\begin{frame}{3-State Markov Chain: Convergence}
  \theorembox{Irreducible, aperiodic chain with transition matrix $P$ has a unique stationary distribution $\pi$ with $\pi^\top P=\pi^\top$.}
  \begin{itemize}
...
\begin{frame}{Antithetic Variates}
 \ideabox{Use negatively correlated pairs $(U,1-U)$ for $U\sim\mathrm{Unif}(0,1)$ and average estimates.}
  \begin{itemize}
...
\begin{frame}{Control Variates}
  \ideabox{With control $g$ s.t. $\mathbb E[g]=\gamma$ known, use $\; \hat I_{cv}=\bar f - \beta(\bar g-\gamma)$ with $\beta^*=\operatorname{Cov}(f,g)/\operatorname{Var}(g)$.}
  \begin{itemize}
...
\begin{frame}{Sensitivity Near Criticality}
  \begin{itemize}
    ...
    \item Example: $\; \partial L/\partial\lambda = \dfrac{1}{\mu(1-\lambda/\mu)^2}$, large for high utilisation.
