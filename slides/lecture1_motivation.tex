...
\begin{frame}{Course Roadmap}
  \begin{itemize}
    \item \textbf{Lecture 1}: Motivation, Poisson recap, simulation mindset.
    \item \textbf{Lecture 2}: Queueing theory basics, Markovian queues, simulation patterns.
    \item \textbf{Lectures 3--4}: Renewal theory, regenerative analysis, heavy tails.
    \item \textbf{Lectures 5--6}: Networks, resource allocation, optimisation under uncertainty.
    \item \textbf{Lectures 7--8}: Project clinics, validation strategies, presentations.
  \end{itemize}
  \vspace{0.6em}
  \textit{We move from first principles to deployable stochastic simulations in eight sessions.}
\end{frame}
...
\begin{frame}{What Stakeholders Care About}
  \begin{itemize}
    \item \textbf{Reliability}: likelihood the system breaches SLA thresholds.
    \item \textbf{Efficiency}: server utilisation vs. customer experience.
    \item \textbf{Elasticity}: ability to scale up/down during seasonal peaks.
    \item \textbf{Fairness}: which users incur delays; equity across customer classes.
    \item \textbf{Resilience}: recovery profile after incidents or spikes.
  \end{itemize}
  \vspace{0.6em}
  \textit{Queueing metrics translate these business objectives into measurable random variables.}
\end{frame}
...
\begin{frame}{Modeling Workflow}
  \begin{enumerate}
    \item \textbf{Observe}: collect arrival/service traces, understand constraints.
    \item \textbf{Idealise}: pick parsimonious assumptions (Poisson arrivals, exponential service, finite buffers).
    \item \textbf{Analyse}: derive quick estimates (Little's Law, stability bounds).
    \item \textbf{Simulate}: stress-test policy choices, explore what-if scenarios.
    \item \textbf{Validate}: compare with real data, refine assumptions, communicate insights.
  \end{enumerate}
  \vspace{0.4em}
  \textit{Each lecture revisits this loop with richer building blocks.}
\end{frame}
...
\begin{frame}{Estimating $\lambda$ in Practice}
  \begin{enumerate}
    \item Choose a consistent window (per minute/hour, per location, per channel).
    \item Compute $\hat{\lambda} = \tfrac{1}{n}\sum X_i$ from historical counts.
    \item Use variance check: Poisson assumption suggests sample variance $\approx \hat{\lambda}$.
    \item Incorporate seasonality via piecewise constant or spline-based rates.
    \item Apply shrinkage/Empirical Bayes for low-volume segments.
  \end{enumerate}
  \vspace{0.4em}
  \textit{These diagnostics appear in the notebook; bring your data to test them.}
\end{frame}

\begin{frame}{Recognising Poisson Limitations}
  \begin{itemize}
    \item \textbf{Overdispersion}: variance $\gg$ mean signals bursty or correlated arrivals.
    \item \textbf{Time-of-day effects}: use non-homogeneous Poisson or piecewise rates.
    \item \textbf{Feedback loops}: arrivals depend on system state (e.g., re-tries) $\Rightarrow$ need queueing networks.
    \item \textbf{Heavy tails}: inter-arrival distributions with long memory require renewal/ARIMA/Hawkes models.
  \end{itemize}
  \vspace{0.4em}
  \textit{Flag these patterns early to avoid mis-specified simulations.}
\end{frame}
...
\begin{frame}{Poisson in Simulation}
  \begin{itemize}
    \item Python: \texttt{numpy.random.poisson(lam, size)} samples counts directly.
    \item For arrival times, sample exponentials: \texttt{np.random.exponential(1/lambda\_rate)}.
    \item In SimPy, schedule arrivals via \texttt{env.timeout(...)} using exponential draws.
...
\begin{frame}{Worked Example: Probability of an Arrival}
  \begin{itemize}
    ...
    \item Probability of zero tickets in that window: $e^{-2} \approx 0.135$.
  \end{itemize}
\end{frame}
...
\begin{frame}{Exercise: Plain Python Warm-Up}
  \begin{enumerate}
    \item Import \texttt{numpy as np}; 
    
    set \texttt{rng = np.random.default\_rng(42)}.
